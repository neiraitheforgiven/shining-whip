    ...
    def __init__(self, game, party, num=None):
        ...
        if num:
            ...
            for unit in self.battleField.units:
                ...
                if self.battleField.unitSpellRank(
                    unit, "Egress"
                ) >= 1 and unit.mp < self.mpCost(unit, 8):
...
class battleField(object):
    ...
    def checkSpells(self, unit, position):
        unit.allowedSpells = {}
        currentTile = self.terrainArray[position]
        if self.unitSpellRank(unit, "Afflict") >= 1 and (
            unit.mp >= self.mpCost(unit, 13)
        ):
        ...
        freezeRank = self.unitSpellRank(unit, "Freeze")
        if freezeRank >= 1 and unit.mp >= self.mpCost(unit, 3):
            self.checkSpell(unit, position, "Freeze I", False, 0, 0)
        if freezeRank >= 2 and (unit.mp >= self.mpCost(unit, 7)):
            self.checkSpell(unit, position, "Freeze II", False, 1, 1)
        if freezeRank >= 3 and (unit.mp >= self.mpCost(unit, 10)):
        ...
        slowRank = self.unitSpellRank(unit, "Slow")
        if slowRank >= 1 and (unit.fp >= self.mpCost(unit, 5)):
